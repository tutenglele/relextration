...
class Params:
    ...
    def __init__(self, ex_index=1, corpus_type='NYT'):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #判断使用device
        ...
        self.downs_en_lr = 1e-4#(3e-5)*5 # (4e-5)*4  4e-4 _NYT89.8(cls直接linear，后pooling)  (4e-5)*4web0.751  改完rel0.781  改drop为0.5 0.75  3e-5*5&改r from so0.808  r提取用bertoutput效果一般0.70， 改用了rel但是潜在r用了cls0.78
        self.clip_grad = 2.
    ...
    def save(self, json_path):
        ...
        """
        params = {}
        if not os.path.exists(self.ex_dir):  # 创建路径
            print("ex_dir does not exist! Making directory {}".format(self.ex_dir))
...
def save_checkpoint(state, is_best, checkpoint):
    ...
    if not os.path.exists(checkpoint): #创建路径
        print("Checkpoint Directory does not exist! Making directory {}".format(checkpoint))
        os.makedirs(checkpoint)
    torch.save(state, filepath) #将模型放在这个文件路径
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'best.pth.tar')) #最好的就复制一份
